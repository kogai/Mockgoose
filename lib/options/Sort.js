    ...
    if (!_.isUndefined(item) && !_.isUndefined(sort)) {
        _.forIn(sort, function (sortOrder, sortKey) {
            ...
            if(!_.isUndefined(itemVal)){
                ...
                } else if (_.isArray(itemVal)) {
                    sortFn = sortAlphaInArray(sortOrder);
                } else {
...
function sortAlphaInArray(sortOrder) {
    return function (a, b) {
        if (sortOrder >= 0) {
            return !_.isUndefined(a) ? a[0].localeCompare(b[0]) : 1;
        } else {
            return !_.isUndefined(b) ? b[0].localeCompare(a[0]) : -1;
        }
    };
}
